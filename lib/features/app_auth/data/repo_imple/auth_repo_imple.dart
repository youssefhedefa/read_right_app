...
class AuthRepoImple implements AuthRepo {
  ...
  Future<Either<Failure, UserCredential>> signUp(
      {required String email, required String password}) async{
    try {
      final res = await authDataSource.register(email: email, password: password);
      return Right(res);
    } catch (e) {
      return Left(
        Failure(
  ...
  @override
  Future<Either<Failure, dynamic>> preSetting({required UserModel user}) async{
    try {
      await authDataSource.createNewUser(user: user);
