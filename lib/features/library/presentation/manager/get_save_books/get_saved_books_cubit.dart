...
class GetSavedBooksCubit extends Cubit<GetSavedBooksState> {
  ...
  Future<void> getSavedBooksList({required List<dynamic> savedBooksId,required Locale locale}) async {
    emit(GetSavedBooksLoading());
    final response = await repo.getNewestBooks(locale: locale);
    response.fold(
      (error) => emit(GetSavedBooksError(error.error.message ?? "Error in getting newest books",)),
      (data) {
